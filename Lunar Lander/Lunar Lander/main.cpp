    ...
    constexpr glm::vec3 INIT_POS_DROPS    = glm::vec3(0.0f, 3.0f, 0.0f);  // Dinosaur starts higher
    constexpr glm::vec3 INIT_SCALE_DROPS  = glm::vec3(1.0f, 1.0f, 0.0f);  // Dinosaur size
    constexpr glm::vec3 INIT_SCALE_BEAKER = glm::vec3(0.5f, 1.0f, 0.0f);  // Cactus size
    constexpr glm::vec3 INIT_POS_BEAKER   = glm::vec3(3.0f, -2.0f, 0.0f);
    
    std::vector<glm::vec3> cactus_positions = {
        glm::vec3(-3.0f, -3.0f, 0.0f), glm::vec3(1.0f, -3.0f, 0.0f), glm::vec3(3.5f, -3.0f, 0.0f)
    };
    ...
    glm::vec3 g_drops_movement = glm::vec3(0.0f, 0.0f, 0.0f);  // Dinosaur falling downward
    ...
    void process_input()
    {
        // Reset horizontal acceleration each frame to zero (gravity is constant)
            ...
            switch (event.type)
            {
                ...
                case SDL_KEYDOWN:
                    switch (event.key.keysym.sym)
                    {
                        ...
                        // Additional keys can be added if needed
                        default:
                            break;
                    }
        ...
        // Adjust horizontal acceleration based on input for A and D keys
        if (key_state[SDL_SCANCODE_A]) {
            m_acceleration.x = -HORIZONTAL_ACCELERATION;  // Accelerate left
        }
        else if (key_state[SDL_SCANCODE_D]) {
            m_acceleration.x = HORIZONTAL_ACCELERATION;   // Accelerate right
        }
    }
    ...
    void update()
    {
        // --- DELTA TIME CALCULATIONS --- //
            ...
            // --- TRANSLATION --- //
            g_drops_matrix = glm::mat4(1.0f);   // Reset transformation matrix
        ...
        for (const auto& cactus_position : cactus_positions) {
            ...
            float y_distance = g_drops_position.y - cactus_position.y;

            // Check for collision only when dinosaur is at ground level
            if (g_drops_position.y <= -3.0f && x_distance < 0.5f && fabs(y_distance) < 0.1f) {
                g_app_status = TERMINATED;  // Mission Failed
                LOG("Mission Failed!");
                return;
            }
        }
        ...
        // --- SUCCESSFUL LANDING --- //
        if (g_drops_position.y <= -3.75f) {  // Adjusted further to ground level
            g_app_status = TERMINATED;  // Mission Success
            LOG("Mission Success!");
        }
    }
    ...
    void render() {
        ...
        // Set up vertices and texture coordinates for objects
        float vertices[] = {
        ...
        // Render desert background closer to the bottom as a narrow strip
        glm::mat4 desert_matrix = glm::mat4(1.0f);
        desert_matrix = glm::translate(desert_matrix, glm::vec3(0.0f, -2.8f, 0.0f)); // Move desert closer to bottom
        desert_matrix = glm::scale(desert_matrix, glm::vec3(10.0f, 3.5f, 1.0f)); // Reduce vertical scale
        draw_object(desert_matrix, g_desert_texture_id);

        // Render cactus obstacles just above the desert strip
        for (const auto& cactus_position : cactus_positions) {
            g_beaker_matrix = glm::mat4(1.0f);
            g_beaker_matrix = glm::translate(g_beaker_matrix, cactus_position + glm::vec3(0.0f, 2.0f, 0.0f)); // Adjust cactus height
            g_beaker_matrix = glm::scale(g_beaker_matrix, INIT_SCALE_BEAKER);
            draw_object(g_beaker_matrix, g_beaker_texture_id); // Render cacti with cactus texture
        }

        // Render falling dinosaur
        draw_object(g_drops_matrix, g_drops_texture_id); // Render dinosaur with falling matrix
